...
class RelopDiagram(Diagram):
    """
        Identifies the relational operators
    """
    ...
    def parse(self, entry: str) -> tuple[int, tuple[str, str]]:
        ...
        elif self._current_state == 9 or self._current_state == 7:
            if entry == "=":
                self._current_lexem += entry
                token_and_lexem = (RELOP_TOKEN, self._current_lexem)
                return_pair = (DiagramProcessing.FINISHED, token_and_lexem)
            else:
                token_and_lexem = (GENERIC_TOKEN, "")
                return_pair =  (DiagramProcessing.FAILED, token_and_lexem)

        elif self._current_state == 4 or self._current_state == 1:
            if entry == "=":
                self._current_lexem += entry
                token_and_lexem = (RELOP_TOKEN, self._current_lexem)
                return_pair = (DiagramProcessing.FINISHED, token_and_lexem)
